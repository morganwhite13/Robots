...
public class Map {
  ...
  private void traceWholeBorder(int xs, int ys) {
    ...
    tmp.addVertex((int)(xs/resolution), (int)(ys/resolution));
      ...
      for (i=0; i<8; i++) {
        if ((xc+xOffset[dp] >= 0) && (xc+xOffset[dp] < width) && (yc+yOffset[dp] >= 0) && (yc+yOffset[dp] < height)) {
          if (binaryGrid[xc+xOffset[dp]][yc+yOffset[dp]] != 0) {
            ...
            tmp.addVertex((int)(xc/resolution), (int)(yc/resolution));
            break;
          }
        }
        dp = (dp + 1)%8;
    ...
    for(int y=height-1; y>0; y--) {
      for (int x=0; x<width; x++) {
        if ((binaryGrid[x][y] == 1) && ((x == 0) || (binaryGrid[x-1][y] == 0))) {
          ...
          // Obstacle tmp = new Obstacle();
          // tmp.addVertex((int)(x/resolution), (int)(y/resolution));
          traceWholeBorder(x, y);
        }
      }
    }
    
    vectorMap.applyLineTolerance(); // apply the line-fitting algorithm
  }
      ...
      for (int x=0; x<width; x++) {
        for (int y=0; y<height; y++) {
          ...
          if (ShowBinary) {
            ...
            else aPen.setColor(Color.white);
          }
