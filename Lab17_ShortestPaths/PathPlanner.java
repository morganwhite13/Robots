//Author is Morgan White SN 101184799 CoAuothor is Haoijin Xun SN: 101071975
...
public class PathPlanner {
  ...
  private void runDijkstrasAlgorithmFrom(Node s) {
    ...
    for (int i = 0; i < visibilityGraph.getNodes().size(); i++) {
      ...
      if (visibilityGraph.getNodes().get(i) != s) {

        visibilityGraph.getNodes().get(i).setDistance(1999999999);//largest number i could do to represent infinity
        visibilityGraph.getNodes().get(i).setPrevious(null);
        q.add(visibilityGraph.getNodes().get(i));
    ...
    while (!q.isEmpty()) {
      ...
      Node curV = q.remove();
      // System.out.println(curV.getDistance());
      for (int i = 0; i < curV.incidentEdges().size(); i++) {

        Edge curEdge = curV.incidentEdges().get(i);

        Node otherV = curEdge.otherEndFrom(curV);
        
        // if (curEdge.getStartNode() == curV) {
        //   otherV = curEdge.getEndNode();
        // } else {
        //   otherV = curEdge.getStartNode();

        // }

        if (otherV.getDistance() > (curV.getDistance() + curEdge.length())) {

          otherV.setDistance(curV.getDistance() + curEdge.length());
          otherV.setPrevious(curV);
    ...
    //   for (int i = 0; i < curV.neighbours().size(); i++) {

    //     // Edge curEdge = curV.neighbours().get(i);

    //     Node otherV = curV.neighbours().get(i);
    //     // if (curEdge.getStartNode() == curV) {
    //     //   otherV = curEdge.getEndNode();
    //     // } else {
    //     //   otherV = curEdge.getStartNode();
    ...
    //     Edge curEdge = new Edge(curV, otherV);
  ...
  private void runShortestTurnPathFrom(Node s) {
    ...
    while (!q.isEmpty()) {
      ...
      for (int i = 0; i < curV.incidentEdges().size(); i++) {
        ...
        if (otherV.getDistance() > (curV.getDistance() + 1)) {

          otherV.setDistance(curV.getDistance() + 1);
          otherV.setPrevious(curV);
