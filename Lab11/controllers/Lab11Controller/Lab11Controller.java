// Author: Morgan White SN: 101184799 & Jerry Liu SN: 101073676
...
public class Lab11Controller {
  ...
  static final double  MAX_SPEED      = 15;    // 19.1 is maximum but it seems that higher values can distort the sensor readings
  ...
  // Apply all sensor readings to the map by using the given MapperApp and the translationField for getting the robot's position
  static void addSensorReadingsToMap(MapperApp mapper, Field translationField) {
    ...
    for (int i=0; i<9; i++) {
      ...
      double objX = x + xOffsets[i]*Math.cos(Math.toRadians(a)) - yOffsets[i]*Math.sin(Math.toRadians(a)) + d*Math.cos(Math.toRadians(a + aOffsets[i]));
      double objY = y + yOffsets[i]*Math.cos(Math.toRadians(a)) + xOffsets[i]*Math.sin(Math.toRadians(a)) + d*Math.sin(Math.toRadians(a + aOffsets[i]));
      ...
      if (d < 20) // This only records on the map if it was within the specified distance
        mapper.applySensorModelReading(x + xOffsets[i]*Math.cos(Math.toRadians(a)) - yOffsets[i]*Math.sin(Math.toRadians(a)), y + yOffsets[i]*Math.cos(Math.toRadians(a)) + xOffsets[i]*Math.sin(Math.toRadians(a)), (int) a+aOffsets[i], d, 2.86, 0.03);
    }
    ...
    for (int i=9; i<14; i++) {
      ...
      if (d < 20) {
        // ADD CODE HERE
   //     mapper.applySensorModelReading(x + xOffsets[i]*Math.cos(Math.toRadians(a)) - yOffsets[i]*Math.sin(Math.toRadians(a)), y + yOffsets[i]*Math.cos(Math.toRadians(a)) + xOffsets[i]*Math.sin(Math.toRadians(a)), (int) a+aOffsets[i], d, 2.86, 0.03);
        mapper.applySensorModelReading(x + xOffsets[i]*Math.cos(Math.toRadians(a)) - yOffsets[i]*Math.sin(Math.toRadians(a)), y + yOffsets[i]*Math.cos(Math.toRadians(a)) + xOffsets[i]*Math.sin(Math.toRadians(a)), (int) a+aOffsets[i], d, 22.9, 0.032);
      }
    }
  }
  ...
  public static void main(String[] args) {
    ...
    MapperApp  mapper = new MapperApp((int)(sizes[0]*100), (int)(sizes[1]*100), false, robot.getDisplay("display"));
    ...
    while (robot.step(timeStep) != -1) {
      ...
      //if (count++ == 1) break;  // UNCOMMENT THIS WHEN ASKED TO IN THE LAB
    }
  }
}
