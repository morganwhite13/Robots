//Author Morgan White SN: 101184799 Coauthor: Clint Galvez SN: 101196386
...
public class Obstacle {
  ...
  private Obstacle convexHull(ArrayList<Point> points) {
    ...
    //   hull.addVertex(points.get(i).x, points.get(i).y);
    ...
    while (pEnd != pStart) {
      ...
      for (int i = 0; i < points.size(); i++) {
        ...
        double tmp = (pEnd.x-pHull.x)*(points.get(i).y-pHull.y) - (pEnd.y-pHull.y)*(points.get(i).x-pHull.x);
  ...
  public Obstacle grow(float radius, int degreeRoundnessAngle) {
    ...
    for (int i =0; i < getVertices().size(); i++) {
      ...
      int x = getVertices().get(i).x;
      int y = getVertices().get(i).y;

      for (int a = 0; a < 360; a += degreeRoundnessAngle) {

        double xoff = (radius/Math.cos(Math.toRadians(degreeRoundnessAngle/2)))*Math.cos(Math.toRadians(a));
        double yoff = (radius/Math.cos(Math.toRadians(degreeRoundnessAngle/2)))*Math.sin(Math.toRadians(a));
        int px = x;
        int py = y;
        ...
        // System.out.println("DDD " + px + " py is " + py);
