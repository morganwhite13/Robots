...
public class PathPlanner {
  ...
  public boolean supportLineIntersectsObstacle(int x, int y, int supportX, int supportY, Obstacle fromObst, ArrayList<Obstacle> obstacles) {
    ...
    if (fromObst != null) {
      ...
      for (int i=0; i<oob.size(); i++) {
        ...
        Point v2 = oob.getVertex((i+1)%oob.size());
  ...
  public ArrayList<Point> getShortestPath() {
    ...
    Node curNode = visibilityGraph.node(end.x, end.y);
    while (curNode != null && curNode != visibilityGraph.node(start.x, start.y)) {
    
      sp.add(0, new Point(curNode.getLocation().x, curNode.getLocation().y));
    ...
    sp.add(0, new Point(start.x, start.y));
    
    // for (int i = 0; i < sp.size(); i++) {

      // System.out.println("x is " + sp.get(i).x + " y is " + sp.get(i).y);
