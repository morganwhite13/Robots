//Author Morgan White SN: 101184799 & Youssef Zayed SN: 101142206
...
public class Map {
  ...
  private void traceWholeBorder(int xs, int ys) {
    ...
    while (true) {
      ...
      for (int i=0; i < xOffset.length && !border; i++) {


        //System.out.println("THING " + (i + d) % 8);
        //if (isInsideGrid(curX + xOffset[(i + d) % 8], curY + yOffset[(i + d) % 8]) && curX + xOffset[(i + d) % 8] >= 0 && curY + yOffset[(i + d) % 8] >= 0) {

        if (curX + xOffset[(i + d) % 8] >= 0 && curY + yOffset[(i + d) % 8] >= 0 && curX + xOffset[(i + d) % 8] < width && curY + yOffset[(i + d) % 8] < height) {

          if (binaryGrid[curX + xOffset[(i + d) % 8]][curY + yOffset[(i + d) % 8]] == 1) {



            binaryGrid[curX + xOffset[(i + d) % 8]][curY + yOffset[(i + d) % 8]] = 2;
            curX += xOffset[(i + d) % 8];
            curY += yOffset[(i + d) % 8];
            border = true;
            // xVals.add(curX);
          ...
          } else if (curX + xOffset[(i + d) % 8] == xs && curY + yOffset[(i + d) % 8] == ys) {
      ...
      if (!border) {
        ...
        //   binaryGrid[xVals.get(i)][yVals.get(i)] = 0;
  ...
  public void computeBorders() {
    ...
    for (int i=height-1; i >= 1; i--) {//idk abotu 1


      for (int j=0; j <= width-1; j++) {

        if (binaryGrid[j][i] == 1 && j != 0 && binaryGrid[j-1][i] == 0) {//x == 0 case idk

          binaryGrid[j][i] = 2;
          traceWholeBorder(j, i);


        } else if (binaryGrid[j][i] == 1 && j == 0) {
  ...
  public void computeBinaryGrid() {
    ...
    for (int i=0; i < occupancyGrid.length; i++) {
      ...
      for (int j=0; j < occupancyGrid[i].length; j++) {
        ...
        if (occupancyGrid[i][j] > BINARY_THRESHOLD) {

          binaryGrid[i][j] = (byte) 1;



        } else {

          binaryGrid[i][j] = (byte) 0;
    ...
    try {
      ...
      for (int x=0; x<width; x++) {
        for (int y=0; y<height; y++) {
          ...
          aPen.fillRect(MapperApp.MARGIN_X/2 + x*magnification, MapperApp.MARGIN_Y/2 + (height*magnification)-(y*magnification), magnification, magnification);
        }
      }
