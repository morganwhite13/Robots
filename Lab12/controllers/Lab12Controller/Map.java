...
public class Map {
  ...
  public void applySensorModelReading(double sensorX, double sensorY, int sensorAngle, double distance, double beamWidthInDegrees, double distanceErrorAsPercent) {
    for (double r = distance*(1-distanceErrorAsPercent); r < distance*(1+distanceErrorAsPercent); r+=0.25) {
      ...
      // for (double a = -beamWidthInDegrees/2; a<=beamWidthInDegrees/2; a += w)  {
        ...
        // if (isInsideGrid(x, y))
          // setTempGridValue(x, y, SIGMA_PROB[angleIndex] * SIGMA_PROB[distIndex]);
          
      // }
      for (double a = 0; a<=beamWidthInDegrees/2; a += w)  {
        ...
        if (r < distance*(1-distanceErrorAsPercent)) {
          ...
          if (isInsideGrid(x, y))
            setTempGridValue(x, y, -SIGMA_PROB[angleIndex]);
