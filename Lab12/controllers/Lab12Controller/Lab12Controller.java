// Author: Morgan White SN: 101184799 & Bill Zhang SN: 101106261
...
public class Lab12Controller {
  ...
  static void addSensorReadingsToMap(MapperApp mapper, Field translationField) {
    ...
    for (int i=0; i<9; i++) {
      ...
      if (d < 30) // This only records on the map if it was within the specified distance
        mapper.applySensorModelReading(x + xOffsets[i]*Math.cos(Math.toRadians(a)) - yOffsets[i]*Math.sin(Math.toRadians(a)), 
                                       y + yOffsets[i]*Math.cos(Math.toRadians(a)) + xOffsets[i]*Math.sin(Math.toRadians(a)), 
                                       (int)(a + aOffsets[i]), 
                                       d,
                                       2.86, 
                                       0.03);
